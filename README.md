...
- [TODO — до полной реализации](#todo--до-полной-реализации-oberon-7)
...
Компиляция происходит **в один файл** (single-file inlining): при обнаружении `IMPORT` компилятор находит, парсит и встраивает экспортированные процедуры, типы, переменные и константы импортированного модуля непосредственно в итоговый `.asm`. Раздельной компиляции и линковки нет — весь код оказывается в одном ассемблерном файле.
...
- **Нет циклических импортов** — модуль A не может импортировать B, если B импортирует A.
- **Нет переименования при импорте** — `IMPORT M := RealName` парсится, но пока не поддерживается.
...
Встроенные процедуры (`INC`, `DEC`, `ABS`, `Out.Int`, `Out.Ln`, `In.Int` и т.д.) и типы (`INTEGER`, `BOOLEAN`, `REAL`, `CHAR`) известны анализатору. При наличии импортированных модулей Sema также проверяет доступность квалифицированных имён (`Math.Abs`).
...
**Записи (`RECORD`)** — поля размещаются последовательно, каждое поле занимает 4 байта (RV32). `TypeDecl` регистрирует тип в реестре с вычислением смещений:
...
Глобальная запись — `.space 8` в `.data`. Локальная — `alloca 8` на стеке.
...
**`NEW(p)`** — выделяет в куче ровно столько байт, сколько занимает тип, на который указывает `p`:
...
| `RECORD(Base)` | Парсинг | Наследование записей распознаётся, но в кодогенерации не применяется |

### Ограничения

- **Нет освобождения памяти** — `DISPOSE` не реализован, нет GC. Для учебных программ в RARS это не критично.
- **Нет проверки на NIL** — разыменование нулевого указателя приведёт к ошибке RARS в runtime.
- **Наследование записей** — `RECORD(Base)` парсится, но поля базового типа не наследуются в кодогенерации.
...
## TODO — до полной реализации Oberon-7

### Высокий приоритет

- [ ] **Наследование записей** — `RECORD(Base)` должен включать поля базового типа, вычислять правильные смещения и поддерживать `IS` (type guard)
- [ ] **Проверка типов в Sema** — полноценная проверка совместимости типов в присваиваниях, вызовах и выражениях (сейчас Sema проверяет имена, но не типы)
- [ ] **`REAL` арифметика** — поддержка `REAL` через инструкции RV32F (`fadd.s`, `fmul.s` и т.д.), конверсия `FLOOR`/`FLT`
- [ ] **`DISPOSE` / GC** — освобождение памяти (простейший mark-and-sweep или free-list allocator)
- [ ] **Множества (`SET`)** — полная поддержка множественных операций (`+`, `-`, `*`, `/`, `IN`) в IR-пайплайне

### Средний приоритет

- [ ] **Раздельная компиляция модулей** — генерация символьных файлов (`.smb`), линковка вместо инлайнинга
- [ ] **Вложенные записи и массивы записей** — `ARRAY N OF RECORD ... END`, доступ `a[i].field`
- [ ] **VAR-параметры записей** — передача записей по ссылке в процедуры
- [ ] **Строки как `ARRAY OF CHAR`** — операции сравнения, копирования, `LEN` для строк
- [ ] **Type guard (`IS`, `(T)`)** — динамическая проверка типа для указателей и VAR-параметров
- [ ] **Проверка границ массивов** — runtime-проверка `0 <= i < LEN(a)` с опциональным отключением

### Низкий приоритет

- [ ] **`REAL` литералы** — корректная IEEE 754 трансляция для `REAL` констант и арифметики
- [ ] **Процедурные типы** — `PROCEDURE` как тип переменной, передача процедур как аргументов
- [ ] **Оптимизации CFG** — слияние блоков, удаление критических рёбер, пропуск пустых блоков
- [ ] **Регистровая аллокация** — замена стековой модели на распределение по регистрам (linear scan / graph coloring)
- [ ] **Peephole-оптимизации** — устранение избыточных `lw`/`sw`, свёртка последовательных `addi`
- [ ] **Хвостовая рекурсия** — замена `call` + `ret` на `jmp` для хвостовых вызовов
- [ ] **Линковка с C** — генерация совместимого ABI для вызова C-функций и библиотек
